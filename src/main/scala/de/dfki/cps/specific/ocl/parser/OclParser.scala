...
import de.dfki.cps.specific
import de.dfki.cps.specific.ocl
import de.dfki.cps.specific.ocl.CollectionKind
import de.dfki.cps.specific.ocl.Expressions.VariableDeclaration
...
trait OclParsers extends PackratParsers with ParserHelpers {
  ...
  def simpleName[T <: NamedElement]: Parser[specific.sysml.SimpleName] = positioned(acceptMatch("simple name", {
    case s: OclTokens.SimpleName if !s.isReserved => specific.sysml.SimpleName(s.chars)
  }))
  ...
  def unreservedSimpleName = acceptMatch("unreserved simple name", {
    case s: OclTokens.SimpleName => specific.sysml.SimpleName(s.chars)
  })
  ...
  def primitiveType: Parser[ResolvedName[Classifier]] =
    ...
    | "Integer" ^^^ specific.sysml.Types.Integer
    | "Real" ^^^ specific.sysml.Types.Real
    | "Unit" ^^^ specific.sysml.Types.Unit
    | "String" ^^^ specific.sysml.Types.String
    | "UnlimitedNatural" ^^^ specific.sysml.Types.UnlimitedNatural) ^^ ResolvedName[Classifier]
  ...
  def oclType: Parser[ResolvedName[Classifier]] =
    ...
    | "OclVoid" ^^^ ocl.Types.VoidType)  ^^ specific.sysml.ResolvedName[Classifier]

  def collectionType: Parser[ResolvedName[Classifier]] =
    collectionTypeIdentifier ~ enclosed(LEFT_PARENS, typeExp, RIGHT_PARENS) ^^ {
      case ct ~ t => specific.sysml.ResolvedName(ocl.Types.collection(ct,t))
    }

  def tupleType: Parser[ResolvedName[Classifier]] =
    "Tuple" ~> enclosed(LEFT_PARENS, variableDeclarationList, RIGHT_PARENS) ^^ ocl.Types.TupleType ^^ specific.sysml.ResolvedName[Classifier]
