  ...
  def load(uri: URI)(implicit resourceSet: ResourceSet): Mappings = {
    ...
    MappingsParsers.phrase(MappingsParsers.file)(tokens) match {
      case MappingsParsers.Success(b: AST.File,_) =>
        ...
        val from = de.dfki.cps.specific.sysml.Model.load(absFrom,"from").getContents.get(0).asInstanceOf[Model]
        val to = de.dfki.cps.specific.sysml.Model.load(absTo,"to").getContents.get(0).asInstanceOf[Model]
